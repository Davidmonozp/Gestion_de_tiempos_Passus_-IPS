...
const CalendarioActividades = ({ actividades, onRecargar }) => {
    ...
    // Forzar actualización del calendario cuando cambian las actividades
    useEffect(() => {
    ...
    const { eventos, leyendaAreas, minutosProgramados, actividadesProgramadas } =
        useMemo(() => {
            const coloresVistos = new Map();

            // 1. Procesar eventos para el calendario
            ...
            // 2. Calcular minutos programados del día
            const minsProg = actividades.reduce((total, act) => {
                const hoy = new Date().toDateString();
                if (new Date(act.created_at).toDateString() === hoy) {
                    return total + (parseInt(act.minutos_planeados) || 0);
                }
                return total;
            }, 0);

            // 3. Filtrar próximas programadas (Debug incluido)
            const prog = actividades
                .filter((act) => {
                    const estado = act.estado
                        ? act.estado.toString().toLowerCase().trim()
                        : "";
                    // Si no ves esto en consola, es que actividades viene vacío
                    return estado === "programada";
                })
                .sort(
                    (a, b) =>
                        new Date(b.fecha_inicio || b.created_at) -
                        new Date(a.fecha_inicio || a.created_at),
                );

            return {
                eventos: evs,
                leyendaAreas: Array.from(coloresVistos.entries()),
                minutosProgramados: minsProg,
                actividadesProgramadas: prog,
            };
        }, [actividades]);
        ...
        tippy(info.el, {
            content: `
                <div style="text-align: left; padding: 5px; color: white;">
                    ...
                    <small>Asignado a: ${asignadoNombre}</small>
    ...
    return (
        <div className="calendar-container">
            ...
            <div className="main-layout-container">
                <div className="calendar-wrapper">
                    <FullCalendar
                        ...
                        locale="es"
                        events={eventos}
                        eventClick={(info) => navigate(`/ver-actividad/${info.event.id}`)}
                    ...
                    <div className="indicadores">
                        ...
                        {resumenDia ? (
                            <div className="resumen-detalle">
                                {horaInicio && (
                                    <p>
                                        🕒 Inicio del turno: <b>{horaInicio}</b>
                                    </p>
                                )}
                                <p>
                                    ⏱️ Tiempo trabajado <b>{tiempoTranscurrido}</b>
                                </p>
                                <p>
                                    📅 Tiempo programado{" "}
                                    <b>{formatoTiempo(minutosProgramados)}</b>
                                </p>
                                <p>
                                    📋 Tiempo ejecutado <b>{resumenDia.tiempo_actividades}</b>
                                </p>
                            ...
                            {actividadesProgramadas.length > 0 ? (
                                actividadesProgramadas.map((act) => (
                                    <div
                                        ...
                                        onClick={() => navigate(`/ver-actividad/${act.id}`)} // <--- ESTO HARÁ LA NAVEGACIÓN
                                        style={{ cursor: 'pointer' }} // <--- ESTO MUESTRA AL USUARIO QUE ES CLICKEABLE
                                    >
            ...
            <div className="container-creacion">
                <CrearActividadCalendario onActividadCreada={onRecargar} />
                <div className="planner-dashboard-card">
                    <div className="planner-content">
                        <div className="planner-text">
                            <h3>Planeador de Actividades</h3>
                            <span className="highlight">
                                Crea actividades <br /> recurrentes fácilmente
                            </span>
                        </div>
                        <Link to="/crear-actividades-recurrentes">
                            <button className="btn-open-planner">Abrir planeador</button>
                        </Link>
                    </div>
...
//     const recargarCalendario = async () => {
...
//                     <CrearActividadCalendario onActividadCreada={recargarCalendario} />
//                 </div>
//                 <div className="planner-dashboard-card">
//                     <div className="planner-content">
//                         <div className="planner-text">
